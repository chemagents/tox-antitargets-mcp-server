...
```bash
cd mcp-servers/tox-antitargets-mcp-server
cp .env.example .env
uv sync
uv run python -m server.tox_server     # serves http://0.0.0.0:7331/mcp
```

## Run with Docker (alongside the other CoScientist MCP servers)

```bash
cd mcp-servers
cp tox-antitargets-mcp-server/.env.example tox-antitargets-mcp-server/.env
docker compose up -d --build tox-antitargets-mcp-server   # host port 7335 -> container 7331
```

For the persistent local MinIO production smoke, use the pinned overlay from `mcp-servers/`:

```bash
docker compose -f docker-compose.yml -f docker-compose.minio.yml up -d --build --wait \
  minio minio-init tox-antitargets-mcp-server heracleum-tox-mcp-server \
  cannabis-biopesticide-mcp-server
tox-antitargets-mcp-server/.venv/bin/python smoke_minio_artifacts.py
```

The smoke calls one real figure tool per server, downloads each public presigned URL, and checks
PNG magic, HTTP/S3 content metadata, service prefix and SHA-256 without printing bearer URLs.
...
```bash
uv run pytest tests -v            # 11 reproduction checks (incl. the ~15s Butina test)
uv run pytest tests -v -m "not slow"
```

## License / data

The dataset is released by the paper authors at github.com/chemagents/ld50-antitargets.
Please cite Nikitin et al. (2025) when using these results.
